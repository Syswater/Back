...
export class CustomerController {
    ...
    @Get('findAll')
    async findAll(@Query("filter") filter?: string, @Query("with_notes") with_notes?: string, @Query("with_order") with_order?: string, @Query("distribution_id") distribution_id?: string, @Query("route_id") route_id?: string): Promise<CustomerDto[]> {
        return await this.customerService.getCustomers({filter, with_notes: with_notes?.toLowerCase() === 'true', with_order: with_order?.toLowerCase() === 'true', distribution_id: parseInt(distribution_id, 10), route_id: route_id?parseInt(route_id, 10):undefined});
    }
