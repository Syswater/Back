...
export class UserService {
    ...
    public async createUser(input: CreateUserInput) {
        ...
        if (existingUser) {
            throw new UserError(UserErrorCode.EXISTING_USERNAME, `El nombre de usuario ya se encuentra registrado en la base de datos:  ${username}`);
        } else {
            return this.toDto(await this.prisma.user.create({
                data: {
                    cellphone,
                    name,
                    password,
    ...
    async delete(id: number): Promise<UserDto> {
        try {
            ...
            if (!user) {
                throw new UserError(UserErrorCode.USER_NOT_FOUND, `No se encuentra la ruta con el id ${id}`);
            } else {
                const deleted = await this.prisma.$transaction(async (tx) => {
                    await tx.user.update({ where: { id: user.id }, data: { username: null } })
    ...
    async update(input: UpdateUserInput) {
        ...
        return this.toDto(await this.prisma.user.update({
            where: { id }, data: {
                cellphone,
                name,
